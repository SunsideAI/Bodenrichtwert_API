...
async function fetchMarktdaten(
  state: string,
  city: string,
  county?: string,   // Landkreis-Fallback (z.B. "Landkreis Gifhorn")
...
function parseBewertungInput(body: Record<string, any>): BewertungInput {
  return {
    art: body.art || null,
...
function buildBericht(
  ...
  const ausstattungScoreLabels: Record<string, string> = {
    '5': 'Stark gehoben',
    '4': 'Gehoben',
    '3': 'Mittel',
    '2': 'Einfach',
  ...
  function scoreToLabel(raw: string | null, labels: Record<string, string>): string {
