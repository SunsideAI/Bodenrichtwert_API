...
export class NRWAdapter implements BodenrichtwertAdapter {
  ...
  private parseXmlResponse(xml: string, layer: string): NormalizedBRW | null {
  ...
  private extractNumberFromXml(xml: string, fields: string[]): number | null {
  ...
  private extractFieldFromXml(xml: string, fields: string[]): string | null {
    for (const field of fields) {
