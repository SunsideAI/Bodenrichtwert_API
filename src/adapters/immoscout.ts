...
export class ImmoScoutAdapter implements BodenrichtwertAdapter {
  ...
  async getBodenrichtwert(lat: number, lon: number): Promise<NormalizedBRW | null> {
    ...
    let prices = await scrapeImmoScoutAtlas(
    ...
    if (!prices) {
      ...
      const searchPrices = await scrapeImmoScoutSearch(
        location.bundeslandSlug, kreisSlug, location.stadtSlug, location.stadt,
      );
      if (searchPrices) {
        prices = searchPrices;
