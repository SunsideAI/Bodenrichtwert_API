 ...
 */
export class ThueringenAdapter implements BodenrichtwertAdapter {
  state = 'Thüringen';
  stateCode = 'TH';
  isFallback = false;

  private wfsUrl = 'https://www.geoproxy.geoportal-th.de/geoproxy/services/boris/vBORIS_simple_wfs';
  ...
  private async tryGmlQuery(lat: number, lon: number): Promise<NormalizedBRW | null> {
    ...
    for (const typeName of ['BODENRICHTWERTZONE', 'bodenrichtwertzone', 'brw:BodenrichtwertZone']) {
      try {
        ...
        if (xml.includes('ExceptionReport') || xml.includes('ServiceException')) continue;
        if (xml.includes('numberReturned="0"') || xml.includes('numberOfFeatures="0"')) continue;
        if (xml.trim().length < 100) continue;

        const wert = this.extractGmlValue(xml, ['BRW', 'brw', 'bodenrichtwert', 'BODENRICHTWERT', 'RICHTWERT']);
        if (!wert || wert <= 0 || wert > 500000) continue;
  ...
  private extractFromFeatures(features: any[]): NormalizedBRW | null {
    // Prefer Wohnbau
    const wohn = features.find((f: any) => {
