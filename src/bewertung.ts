...
const NATIONAL_AVG_QM_PREIS = { haus: 2200, wohnung: 2800 } as const;
 ...
 */
const STATE_AVG_QM_PREIS: Record<string, { haus: number; wohnung: number }> = {
...
const DEFAULT_WOHNFLAECHE: Record<string, number> = {
 ...
 */
function estimateWohnflaeche(
  input: BewertungInput,
): { wohnflaeche: number; hinweis: string } {
  ...
  let key = 'efh'; // Default
  if (art.includes('wohnung') || o.includes('etw') || o.includes('eigentum')) key = 'etw';
  else if (o.includes('mehrfamilien') || o.includes('mfh')) key = 'mfh';
  else if (o.includes('zweifamilien') || o.includes('zfh')) key = 'zfh';
  else if (o.includes('doppelhaush') || o.includes('dhh')) key = 'dhh';
  else if (o.includes('reihenmittel') || o.includes('rmh')) key = 'rmh';
  else if (o.includes('reihenend') || o.includes('reh')) key = 'reh';
 ...
 */
function estimateGrundstuecksflaeche(
  wohnflaeche: number,
  istHaus: boolean,
): { grundflaeche: number; hinweis: string } {
 ...
 */
function mapToErtragswertTyp(art: string | null, objektunterart: string | null): 'mfh' | 'etw' | 'efh' | 'zfh' {
  const a = (art ?? '').toLowerCase();
  const o = (objektunterart ?? '').toLowerCase();

  if (a.includes('wohnung') || o.includes('etw') || o.includes('eigentum')) return 'etw';
  if (o.includes('mehrfamilien') || o.includes('mfh')) return 'mfh';
  if (o.includes('zweifamilien') || o.includes('zfh')) return 'zfh';
  return 'efh';
}

function determineConfidenceAndSpread(
  brw: NormalizedBRW | null,
  methode: 'sachwert-lite' | 'marktpreis-indikation' | 'vergleichswert',
  hasErtragswert: boolean,
  ewDivergenz?: number | null,
): { konfidenz: 'hoch' | 'mittel' | 'gering'; spread: number } {
  let konfidenz: 'hoch' | 'mittel' | 'gering';
  let spread: number;

  if (methode === 'marktpreis-indikation') {
    konfidenz = 'mittel'; spread = 0.15;
  } else if (methode === 'vergleichswert') {
    if (brw?.schaetzung) { konfidenz = 'mittel'; spread = 0.12; }
    else if (hasErtragswert) { konfidenz = 'hoch'; spread = 0.06; }
    else { konfidenz = 'hoch'; spread = 0.10; }
  } else {
    if (!brw || brw.wert <= 0) { konfidenz = 'gering'; spread = 0.20; }
    else if (brw.schaetzung) { konfidenz = 'mittel'; spread = 0.15; }
    else if (hasErtragswert) { konfidenz = 'hoch'; spread = 0.06; }
    else { konfidenz = 'hoch'; spread = 0.08; }
  }

  // Methoden-Divergenz senkt die Konfidenz
  if (ewDivergenz != null && ewDivergenz > 0.30) {
    if (konfidenz === 'hoch') konfidenz = 'mittel';
...
export function buildBewertung(
  ...
  if (wohnflaeche < 15 && !wohnflaecheGeschaetzt) {
    validationHinweise.push(
      `Wohnfläche ${wohnflaeche} m² ist ungewöhnlich klein. Ergebnis möglicherweise unzuverlässig.`,
    );
  }
  if (wohnflaeche > 2000) {
    validationHinweise.push(
      `Wohnfläche ${wohnflaeche} m² ist ungewöhnlich groß. Ergebnis möglicherweise unzuverlässig.`,
    );
  }
  ...
  const bewertungsmethode: 'sachwert-lite' | 'marktpreis-indikation' | 'vergleichswert' =
    istWohnung && marktPreisProQm ? 'vergleichswert'
  ...
  let ertragswertErgebnis: number | null = null;
  let is24Gesamtwert: number | null = null;
  ...
  if (bewertungsmethode === 'vergleichswert') {
    ...
    if (mietPreisProQm && mietPreisProQm > 0) {
      ...
      if (ertragswertResult && ertragswertResult.ertragswert > 0) {
        ...
        datenquellen.push(
          'Vergleichswertverfahren (ImmoWertV § 15)',
          'ImmoScout24 Atlas Marktpreise',
          'Ertragswertverfahren (ImmoWertV §§ 27-34)',
        );
        hinweise.push(
          `Vergleichswert (${vergleichswert.toLocaleString('de-DE')} €) gewichtet mit Ertragswert ${ertragswertErgebnis.toLocaleString('de-DE')} € (80/20, LiZi ${(ertragswertResult.liegenschaftszins * 100).toFixed(1)}%).`,
        );
      } else {
    ...
    if (hasBRW) datenquellen.push('BORIS/WFS Bodenrichtwert');
    ...
    if (marktPreisProQm) {
      ...
      // Blend
      realistischerImmobilienwert = Math.round(
        is24Gesamtwert * (1 - nhkWeight) + nhkSachwert * nhkWeight,
      );
      gebaeudewert = Math.round(Math.max(0, realistischerImmobilienwert - bodenwert));

      datenquellen.push('ImmoScout24 Atlas Marktpreise');
      hinweise.push(
        `Sachwert-Blend: ${Math.round(nhkWeight * 100)}% NHK (${nhkSachwert.toLocaleString('de-DE')} €) / ${Math.round((1 - nhkWeight) * 100)}% Markt (${is24Gesamtwert.toLocaleString('de-DE')} €)${hatStadtteil ? ' [Stadtteil-Daten]' : ' [Stadt-Durchschnitt]'}.`,
      );
      ...
      if (bundesland && wohnflaeche > 0) {
        ...
        if (stateAvg) {
          ...
          if (stateRefWert > nhkSachwert * 1.10) {
            ...
            hinweise.push(
              `NHK-Sachwert (${nhkSachwert.toLocaleString('de-DE')} €) liegt unter Landesdurchschnitt. Blend: 70% NHK / 30% Landesdurchschnitt (${Math.round(stateRefWert).toLocaleString('de-DE')} €).`,
            );
          }
        }
      }
    ...
    if (mietPreisProQm && mietPreisProQm > 0 && bodenwert > 0 && hasBRW) {
      ...
      if (ertragswertResult && ertragswertResult.ertragswert > 0) {
        ...
        if (abweichung > 0.30) {
          hinweise.push(
            `Ertragswert (${ertragswertResult.ertragswert.toLocaleString('de-DE')} €) weicht ${Math.round(abweichung * 100)}% vom Sachwert ab. Ertragswert-Details: Rohertrag ${ertragswertResult.jahresrohertrag.toLocaleString('de-DE')} €/J., Liegenschaftszins ${(ertragswertResult.liegenschaftszins * 100).toFixed(1)}%.`,
          );
        } else {
          hinweise.push(
            `Ertragswert bestätigt Bewertung: ${ertragswertResult.ertragswert.toLocaleString('de-DE')} € (Abweichung ${Math.round(abweichung * 100)}%, LiZi ${(ertragswertResult.liegenschaftszins * 100).toFixed(1)}%, V=${ertragswertResult.vervielfaeltiger}).`,
          );
        }
      }
    }
    ...
    if (
      ertragswertErgebnis != null && ertragswertErgebnis > 0 &&
      ...
      if (ewIs24Div < 0.20 && ewSwDiv > 0.25 && is24SwDiv > 0.25) {
        ...
        realistischerImmobilienwert = Math.round(
          ertragswertErgebnis * 0.50 + is24Gesamtwert * 0.30 + vorher * 0.20,
        );
        gebaeudewert = Math.max(0, realistischerImmobilienwert - bodenwert);
        hinweise.push(
          `3-Methoden-Blend: EW (${ertragswertErgebnis.toLocaleString('de-DE')} €) und IS24 `
  ...
  for (let iteration = 0; iteration < 3; iteration++) {
    ...
    if (!appliedSignals.has(1) && istHaus && gebaeudewert > 0 && wohnflaeche > 0) {
      ...
      if (gebWertProQm > maxPlausibel) {
        ...
        hinweise.push(
          `Plausibilitätskorrektur: Gebäudewert/m² (${Math.round(gebWertProQm)} €) übersteigt Maximum (${maxPlausibel} €/m²). Korrigiert.`,
        );
    ...
    if (!appliedSignals.has(3) && istHaus && realistischerImmobilienwert > 0 && bodenwert > 0) {
      ...
      if (bodenwertAnteil > 0.70) {
        ...
        if (realistischerImmobilienwert < minGesamt) {
          hinweise.push(
            `Plausibilitätskorrektur: Bodenwertanteil (${Math.round(bodenwertAnteil * 100)}%) zu hoch für bebautes Grundstück. Gesamtwert angehoben.`,
          );
