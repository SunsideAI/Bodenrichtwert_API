 ...
 */
export function slugifyAscii(name: string): string {
  return name
    .toLowerCase()
    .replace(/ä/g, 'ae')
    .replace(/ö/g, 'oe')
...
async function mobileSearch(geocode: string, realestatetype: string): Promise<SearchListing[]> {
  ...
  try {
    ...
    for (const entry of resultListItems) {
      ...
      const attrs: any[] = expose?.attributes || [];
      ...
      if (!price || !area) {
        const sections: any[] = expose?.sections || [];
        for (const section of sections) {
          const sectionAttrs: any[] = section?.attributes || [];
      ...
      if (!price) {
        ...
        if (p) price = typeof p === 'number' ? p : parseGermanNumber(String(p).replace(/[^\d,.]/g, ''));
      }
      if (!area) {
        const a = expose?.livingSpace ?? expose?.livingArea ?? expose?.area;
        if (a) area = typeof a === 'number' ? a : parseGermanNumber(String(a).replace(/[^\d,.]/g, ''));
      }
...
export async function scrapeImmoScoutSearch(
  ...
  let hausListings: SearchListing[] = [];
  let wohnungListings: SearchListing[] = [];
