...
let _discoveredLayers: Record<string, string[]> = {};
 ...
 */
export async function fetchImmobilienrichtwert(
  lat: number,
  lon: number,
  teilmarktFilter?: string,
): Promise<NRWImmobilienrichtwert | null> {
...
async function queryIrwEndpoint(
  lat: number,
  lon: number,
  wmsUrl: string,
  teilmarktFilter?: string,
  ...
  const xmlResults = await Promise.allSettled(
    layersToTry.map(layer => queryWmsXml(lat, lon, wmsUrl, layer, teilmarktFilter)),
  );
...
async function discoverLayers(wmsUrl: string): Promise<string[]> {
  try {
...
async function queryWmsLayer(
  ...
  let results: NRWImmobilienrichtwert[];
...
function parseJsonResponse(text: string): NRWImmobilienrichtwert[] {
  try {
    ...
    return features
      ...
      .filter(Boolean) as NRWImmobilienrichtwert[];
  } catch {
    return [];
  }
}

function parseXmlResponse(xml: string): NRWImmobilienrichtwert[] {
...
function parseHtmlResponse(html: string): NRWImmobilienrichtwert[] {
...
function parseGermanNumber(numStr: string): number | null {
  if (numStr.includes(',')) {
...
function extractNumberFromXml(xml: string, fields: string[]): number | null {
...
function extractFieldFromXml(xml: string, fields: string[]): string | null {
  for (const field of fields) {
