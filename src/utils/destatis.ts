 ...
 */
export async function fetchBaupreisindex(): Promise<BaupreisindexResult> {
  try {
...
async function fetchFromGenesis(): Promise<BaupreisindexEntry[]> {
 ...
 */
function parseFfcsv(csv: string): BaupreisindexEntry[] {
  ...
  const zeitIdx = header.findIndex((h) =>
    /zeit|time|quartal|period/i.test(h),
  );
  const wertIdx = header.findIndex((h) =>
    /wert|value|index|__/i.test(h) || h === '',
  );

  // Alle numerischen Spalten finden (für den Fall dass die Spalten anders benannt sind)
  const entries: BaupreisindexEntry[] = [];
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const isWohngebaeude =
      lineStr.includes('wohngebäude') ||
