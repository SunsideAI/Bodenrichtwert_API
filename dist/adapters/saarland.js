 ...
 */
export class SaarlandAdapter {
    state = 'Saarland';
    stateCode = 'SL';
    isFallback = false;
    // Mapbender WMS – primärer Endpunkt (hat BORISSL2024 im Test gefunden)
    mapbenderUrl = 'https://geoportal.saarland.de/mapbender/php/wms.php';
    mapbenderParams = { inspire: '1', layer_id: '48720', withChilds: '1' };
    ...
    mapservUrl = 'https://geoportal.saarland.de/gdi-sl/mapserv';
    // Bekannte Layer-Kandidaten (STEUER_BORIS bestätigt via Test)
    knownLayers = ['STEUER_BORIS', 'BORISSL2024', 'BORISSL2022', 'BORIS_SL', 'Bodenrichtwerte', 'bodenrichtwerte', 'BRW'];
    discoveredMapbenderLayers = null;
    discoveredMapservLayers = {};
    ...
    // ─── Mapbender WMS ─────────────────────────────────────────────────────────
    async queryMapbender(lat, lon) {
        if (!this.discoveredMapbenderLayers) {
            this.discoveredMapbenderLayers = await this.discoverLayers(this.mapbenderUrl, this.mapbenderParams);
    ...
    // ─── MapServ WMS ───────────────────────────────────────────────────────────
    async queryMapserv(lat, lon, mapFile) {
    ...
    async queryWmsWithStrategies(lat, lon, baseUrl, extraParams, layers) {
        for (const layer of layers) {
            // Strategie 1: WMS 1.1.1 + EPSG:25832 (native UTM-Koordinaten)
            for (const fmt of ['text/plain', 'text/xml', 'text/html']) {
                try {
    ...
    // ─── WMS Query ─────────────────────────────────────────────────────────────
    async queryWms(lat, lon, baseUrl, extraParams, layer, infoFormat, srs, version) {
        let bbox;
        if (srs === 'EPSG:25832') {
        ...
        if (text.includes('<TITLE>MapServer Message</TITLE>'))
            return null;
    ...
    // ─── Layer Discovery ───────────────────────────────────────────────────────
    async discoverLayers(baseUrl, extraParams) {
        ...
        for (const version of ['1.1.1', '1.3.0']) {
            try {
                ...
                if (xml.includes('<TITLE>MapServer Message</TITLE>'))
                    continue;
                const layers = [];
                // Queryable Layer
...
//# sourceMappingURL=saarland.js.map