...
export class HamburgAdapter {
    ...
    async tryJsonQuery(typeName, bbox) {
    ...
    async tryGmlQuery(typeName, bbox) {
        const params = new URLSearchParams({
            service: 'WFS',
            version: '2.0.0',
            request: 'GetFeature',
    ...
    async getTypeName() {
        ...
        try {
            ...
            // Priorisierte Suche
            const preferred = typeMatches.find(n => n.toLowerCase().includes('aktuell') || n.toLowerCase().includes('bodenrichtwert'));
