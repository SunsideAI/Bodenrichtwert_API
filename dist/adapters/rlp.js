 ...
 */
export class RheinlandPfalzAdapter {
    state = 'Rheinland-Pfalz';
    stateCode = 'RP';
    isFallback = false;
    wmsUrl = 'https://geo5.service24.rlp.de/wms/genbori_rp.fcgi';
    // Layer-Priorität: Wohnbau zuerst, dann gemischt
    layers = ['Wohnbauflaechen', 'Gemischte_Bauflaechen'];
    ...
    async queryWmsLayer(lat, lon, layer) {
    ...
    async queryWmsHtml(lat, lon, layer) {
        const delta = 0.001;
        const bbox = `${lon - delta},${lat - delta},${lon + delta},${lat + delta}`;
        const params = new URLSearchParams({
            SERVICE: 'WMS',
