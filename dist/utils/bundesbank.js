 ...
 */
// ─── API-Abruf ───────────────────────────────────────────────────────────────
const BUNDESBANK_API_URL = 'https://api.statistiken.bundesbank.de/rest/data/BBK01/BBSRI';
/**
 ...
 */
export async function fetchPreisindex() {
    try {
 ...
 */
function parseSdmxResponse(json) {
    try {
        const observations = findObservations(json);
        if (!observations)
            return [];
        // Zeitdimensionen extrahieren
        const timeDimension = json?.structure?.dimensions?.observation?.find((d) => d.id === 'TIME_PERIOD');
 ...
 */
function findObservations(json) {
 ...
 */
export function calcIndexKorrektur(stichtag, indexData) {
    if (!indexData.length)
        return null;
    const stichtagDate = new Date(stichtag);
    if (isNaN(stichtagDate.getTime()))
        return null;
    ...
    if (stichtagIndex == null || aktuellerIndex == null)
        return null;
    if (stichtagIndex === 0)
        return null; // Division by zero vermeiden
 ...
 */
function findClosestIndex(quartal, indexData) {
    // Exakter Match
