...
function buildCacheKey(adresse, art) {
 ...
 */
export function checkResearchTriggers(brw, marktdaten, bundesland) {
...
// ─── Prompt-Bau ────────────────────────────────────────────────────────────
function buildResearchPrompt(adresse, bundesland, art, objektunterart, wohnflaeche, baujahr, triggers) {
...
// ─── Response-Parsing ──────────────────────────────────────────────────────
function parseResearchResponse(text, citations, durationMs, triggers) {
    ...
    try {
        ...
        if (Array.isArray(parsed.quellen)) {
            for (const q of parsed.quellen) {
                if (typeof q === 'string' && q.startsWith('http'))
                    quellen.add(q);
            }
        }
 ...
 */
export async function performResearch(adresse, bundesland, art, objektunterart, wohnflaeche, baujahr, triggers) {
    // Cache prüfen
    ...
    try {
        ...
        console.log(`KI-Recherche abgeschlossen (${durationMs}ms): BRW=${result.recherchierter_brw}, qm=${result.vergleichspreis_qm}, Quellen=${result.quellen.length}`);
        return result;
    }
    catch (err) {
        const durationMs = Date.now() - startTime;
        const isTimeout = err?.name === 'AbortError';
        console.warn(`KI-Recherche fehlgeschlagen (${durationMs}ms):`, isTimeout ? 'Timeout' : err?.message || err);
        return {
            recherchierter_brw: null,
            vergleichspreis_qm: null,
            vergleichsobjekte_anzahl: null,
...
//# sourceMappingURL=ai-research.js.map