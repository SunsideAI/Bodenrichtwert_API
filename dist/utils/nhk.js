...
let _gmbMAFCache = null;
let _gmbMAFLoaded = false;
function loadGMBSachwertfaktoren() {
    if (_gmbMAFLoaded)
        return _gmbMAFCache;
    _gmbMAFLoaded = true;
    try {
 ...
 */
export function lookupMAF(vorlaeufigSachwert, brwProQm, bundesland) {
    // 1. Versuch: GMB-Daten
    if (bundesland) {
        ...
        if (gmb?.bundeslaender[bundesland]) {
            ...
            for (const entry of blData.daten) {
                ...
                if (numbers.length >= 2) {
                    ...
                    // "bis X" oder "ab X"
                    if (entry.segment.includes('bis') && vorlaeufigSachwert <= numbers[0])
                        return entry.faktor;
                    if (entry.segment.includes('ab') && vorlaeufigSachwert >= numbers[0])
                        return entry.faktor;
                }
            }
        }
 ...
 */
export function calcGebaeudewertNHK(wohnflaeche, baujahr, objektunterart, ausstattung, modernisierung, istHaus = true, brwProQm = 100, externalBpi, bundesland) {
    const hinweise = [];
