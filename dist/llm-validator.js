...
function buildCacheKey(input, bewertung) {
    const sig = [
        input.art,
        input.baujahr,
        input.wohnflaeche,
...
// ─── Prompt-Bau ─────────────────────────────────────────────────────────────
function buildPrompt(input, bewertung, brw, adresse, bundesland) {
...
async function callClaude(prompt) {
    ...
    try {
        ...
        if (!textBlock)
            throw new Error('Keine Text-Antwort vom LLM');
...
// ─── Response-Parsing ───────────────────────────────────────────────────────
function parseResponse(raw, durationMs) {
 ...
 */
export async function validateBewertung(input, bewertung, brw, adresse, bundesland) {
    ...
    try {
        ...
        console.log(`LLM-Validierung: ${result.status} (confidence: ${result.confidence}, ${durationMs}ms)`);
        return result;
    }
    catch (err) {
        const durationMs = Date.now() - startTime;
        const isTimeout = err?.name === 'AbortError';
        console.warn(`LLM-Validierung fehlgeschlagen (${durationMs}ms):`, isTimeout ? 'Timeout' : err?.message || err);
        return {
            status: 'fehler',
            confidence: 0,
 ...
 */
export function applyLLMCorrection(bewertung, validation) {
    ...
    // Graduierter Blend
    const korrigierterWert = Math.round(originalWert * (1 - blendWeight) + llmWert * blendWeight);
...
export function validationCacheStats() {
